...
func init() {
	android.RegisterModuleType("flare_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = flareExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = flareExpandVariables(ctx, dep_file)
	...
	cmd := flareExpandVariables(ctx, String(g.properties.Cmd))
